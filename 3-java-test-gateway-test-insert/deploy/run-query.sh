...
java \
  ...
  -Dotel.service.name=test-query \
  -Dotel.exporter.otlp.endpoint="${OTEL_EXPORTER_OTLP_ENDPOINT}" \
  -Dotel.exporter.otlp.protocol="${OTEL_EXPORTER_OTLP_PROTOCOL}" \
  -Dotel.resource.attributes="${OTEL_RESOURCE_ATTRIBUTES},host.name=sample-host-b,service.role=database-reader" \
  -jar test-query/target/test-query-0.0.1-SNAPSHOT.jar
